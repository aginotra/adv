...
class AdvertiserBusiness extends Model
{
    protected $table = "adv_buss_detail";
    ...
    public function imageUpload($imageBase64Data, $uploadDir)
    {
        ...
        $img_hash = $base_url . '/img/adv/' . $time . '.png';
        return $img_hash;
    }

    public function getDetailByUserId($userId)
    {
        $userId = DB::table('adv_buss_detail')->where('user_id', $userId)->get();
        return $userId;
    }

    public function insert($data)
    {
        $user = new AdvertiserBusiness();
